...
function __ProcError(){
	...
	this._before = null;	// 前の計算エラー情報
	this._next   = null;	// 次の計算エラー情報
}

// 計算エラー情報管理クラス
function _ProcError(){
	this._top = null;	// 計算エラー情報リスト
	this._end = null;	// 計算エラー情報リスト
	this._num = 0;		// 計算エラー情報の個数
}
...
_ProcError.prototype = {
	...
	// 計算エラー情報を確認する
	get : function( index, err/*_Integer*/, num/*_Integer*/, func/*_String*/, token/*_String*/ ){
		var tmp = 0;
		var cur = this._top;
		while( true ){
			if( cur == null ){
